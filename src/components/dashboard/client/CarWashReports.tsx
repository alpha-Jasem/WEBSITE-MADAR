...
export function CarWashReports() {
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(20)
      doc.text('Madar AI - Car Wash Report', 105, 18, { align: 'center' })
      doc.setFontSize(11)
      doc.setTextColor(148, 163, 184)
      doc.text(`${companyName}  |  ${dateStr}`, 105, 28, { align: 'center' })

      // Stats section
      doc.setTextColor(30, 30, 30)
      doc.setFontSize(14)
      doc.text('Summary - Last 30 Days', 14, 52)

      const rows = [
        ['Cars Today', String(stats.todayVisits)],
        ['Cars This Month', String(stats.monthVisits)],
        ['Monthly Revenue (SAR)', stats.revenue > 0 ? `${stats.revenue.toLocaleString()} SAR` : '—'],
        ['Loyalty Rewards', String(stats.milestones)],
      ...
      if (topCustomers.length > 0) {
        ...
        doc.setFontSize(14)
        doc.setTextColor(30, 30, 30)
        doc.text('Top Loyal Customers', 14, y)
        y += 8
        doc.setFontSize(10)
        for (const c of topCustomers.slice(0, 8)) {
          doc.setTextColor(100, 100, 100)
          doc.text(c.name || c.phone, 14, y)
          doc.setTextColor(30, 30, 30)
          doc.text(`${c.total_visits} visits`, 140, y)
          doc.setDrawColor(220, 220, 220)
          doc.line(14, y + 2, 196, y + 2)
          y += 8
          if (y > 270) break
      ...
      doc.text('Generated by Madar AI Agency OS', 105, 287, { align: 'center' })
