...
export function CarWashMemberships() {
  ...
  const featureEnabled = can.memberships || can.wallet
  ...
  const uniquePlans = useMemo(() => {
    ...
    return plans.filter(p => {
      const key = `${p.name.trim()}-${Number(p.price)}-${Number(p.washes_per_month)}-${p.active}`
  ...
  const walletBalance = customers.reduce((sum, c) => sum + Number(c.wallet_balance || 0), 0)
  const recurringRevenue = activeMemberships.reduce((sum, item) => sum + Number(planMap.get(item.plan_id || '')?.price || 0), 0)
  const activePlans = uniquePlans.filter(p => p.active)
  ...
  const journey = [
    ...
    { label: 'المحفظة متاحة كطريقة دفع',         done: can.wallet,      icon: CreditCard, optional: true },
  ]

  const coreJourney = journey.filter(item => !(item as any).optional)
  ...
  const createPlan = async () => {
    ...
    const duplicate = activePlans.some(p =>
      p.name.trim() === planForm.name.trim()
      && Number(p.price) === Number(planForm.price || 0)
      && Number(p.washes_per_month) === Number(planForm.washes || 0)
    )
    if (duplicate) {
    ...
    setSaving(false)
    setFeedback(error ? 'تعذّر إنشاء الباقة' : 'تم إنشاء باقة الاشتراك')
    if (!error) load()
  }
  ...
  const activateMembership = async () => {
    ...
    if (alreadyActive) {
      setFeedback('هذا العميل لديه اشتراك نشط حالياً. لا يمكن تفعيل اشتراك ثانٍ حتى لا يتم احتسابه مرتين.')
      return
    }
    ...
    setSaving(false)
    setFeedback(error ? 'تعذّر تفعيل الاشتراك' : 'تم تفعيل اشتراك العميل')
    if (!error) load()
  }
  ...
  const chargeWallet = async () => {
    ...
    setSaving(false)
    setFeedback(error ? 'تعذّر تحديث المحفظة' : 'تم تحديث رصيد المحفظة')
    if (!error) {
  ...
  if (authLoading || loading) {
    return (
      <div className="madar-center-state" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 300, gap: 10 }}>
        <Loader2 size={20} className="animate-spin" style={{ color: '#22D3EE' }} />
        <span style={{ color: '#475569', fontFamily: 'Tajawal, sans-serif', fontSize: 13 }}>جاري تحميل الاشتراكات...</span>
      </div>
    )
  }
      ...
      <div dir="rtl" className="mx-auto flex min-h-[58vh] max-w-3xl items-center justify-center">
        <section className="w-full rounded-[28px] border border-slate-200 bg-white p-8 text-center shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
          ...
          <p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-slate-500 font-tajawal">
            هذه الصفحة تظهر فقط للمغاسل التي فعّل لها الأدمن ميزات الاشتراكات الشهرية أو المحفظة الرقمية أو الدفع الإلكتروني.
          </p>
        </section>
      </div>
    )
  ...
  return (
    <div dir="rtl" className="space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-slate-500 font-tajawal">
              فعّل باقات غسيل شهرية أو اشحن محفظة العميل. الميزة اختيارية وتتحكم فيها الإدارة لكل مغسلة.
            </p>
          </div>
          ...
          {journey.map(item => (
            <article key={item.label} className={`rounded-2xl border p-4 ${item.done ? 'border-emerald-100 bg-emerald-50/60' : 'border-slate-200 bg-slate-50'}`}>
              ...
              <span className={`mt-2 inline-flex rounded-full px-2 py-1 text-[11px] font-bold font-tajawal ${item.done ? 'bg-emerald-100 text-emerald-700' : 'bg-white text-slate-500'}`}>
                {item.done ? 'جاهز' : (item as any).optional ? 'لاحقاً' : 'ينتظر'}
              </span>
            </article>
          ))}
        </div>
      ...
      <section className="grid grid-cols-1 gap-5 xl:grid-cols-[1fr_1fr]">
        {can.memberships && (
          <Panel title="باقات الاشتراك الشهرية" desc="أنشئ باقات مثل 4 غسلات أو 8 غسلات شهرياً.">
            <div className="grid gap-3 md:grid-cols-[1.2fr_0.7fr_0.7fr_auto]">
              <Input label="اسم الباقة" value={planForm.name} onChange={v => setPlanForm(f => ({ ...f, name: v }))} />
              <Input label="السعر" value={planForm.price} onChange={v => setPlanForm(f => ({ ...f, price: v }))} dir="ltr" />
              <Input label="الغسلات/شهر" value={planForm.washes} onChange={v => setPlanForm(f => ({ ...f, washes: v }))} dir="ltr" />
              <button disabled={saving} onClick={createPlan} className="mt-auto inline-flex h-[43px] items-center justify-center gap-2 rounded-xl bg-gradient-to-l from-[#0B63F6] to-[#00BFFF] px-4 text-sm font-bold text-white shadow-[0_12px_26px_rgba(11,99,246,0.22)] transition hover:-translate-y-px disabled:opacity-50 font-cairo">
                <Plus size={15} />
                إضافة
              </button>
            </div>
            <div className="mt-5 space-y-2">
              {uniquePlans.length === 0 ? <Empty text="لا توجد باقات بعد" /> : uniquePlans.map(p => (
                <div key={p.id} className="flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <div>
                    <strong className="text-sm text-slate-950 font-cairo">{p.name}</strong>
                    <p className="text-xs text-slate-500 font-tajawal">{p.washes_per_month} غسلات / شهر</p>
                  </div>
                  <span className="font-sora text-lg font-black text-slate-950">{money(Number(p.price))} <small className="text-xs text-slate-400">ر.س</small></span>
                </div>
              ))}
            </div>
          </Panel>
        ...
        {can.memberships && (
          <Panel title="تفعيل اشتراك لعميل" desc="اربط العميل بخطة شهرية يدوياً الآن. الدفع الإلكتروني من جوال العميل يبدأ بعد ربط مزود الدفع.">
            <Select label="العميل" value={membershipForm.customerId} onChange={v => setMembershipForm(f => ({ ...f, customerId: v }))}>
              <option value="">اختر العميل</option>
              {customers.map(c => <option key={c.id} value={c.id}>{c.name || phoneLabel(c.phone)} - {phoneLabel(c.phone)}</option>)}
            </Select>
            <Select label="الباقة" value={membershipForm.planId} onChange={v => setMembershipForm(f => ({ ...f, planId: v }))}>
              <option value="">اختر الباقة</option>
              {activePlans.map(p => <option key={p.id} value={p.id}>{p.name} - {money(Number(p.price))} ر.س</option>)}
            </Select>
        ...
        {can.wallet && (
          <Panel title="المحفظة الرقمية" desc="اشحن رصيد العميل أو اخصم منه يدوياً الآن. الدفع الآلي لاحقاً عبر Moyasar.">
            <Select label="العميل" value={walletForm.customerId} onChange={v => setWalletForm(f => ({ ...f, customerId: v }))}>
              <option value="">اختر العميل</option>
              {customers.map(c => <option key={c.id} value={c.id}>{c.name || phoneLabel(c.phone)} - رصيد {money(Number(c.wallet_balance || 0))} ر.س</option>)}
            </Select>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <Input label="المبلغ" value={walletForm.amount} onChange={v => setWalletForm(f => ({ ...f, amount: v }))} dir="ltr" />
              <Input label="ملاحظة" value={walletForm.note} onChange={v => setWalletForm(f => ({ ...f, note: v }))} />
            </div>
...
function Input({ label, value, onChange, dir = 'rtl' }: { label: string; value: string; onChange: (v: string) => void; dir?: 'rtl' | 'ltr' }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-xs font-bold text-slate-500 font-tajawal">{label}</span>
      <input value={value} onChange={e => onChange(e.target.value)} dir={dir} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-950 outline-none focus:border-sky-400 focus:bg-white font-tajawal" />
    </label>
  )
}
...
function Select({ label, value, onChange, children }: { label: string; value: string; onChange: (v: string) => void; children: ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-xs font-bold text-slate-500 font-tajawal">{label}</span>
      <select value={value} onChange={e => onChange(e.target.value)} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-950 outline-none focus:border-sky-400 focus:bg-white font-tajawal">
        {children}
      </select>
    </label>
  )
