...
import { LayoutDashboard, Zap, Users2, BarChart3, Settings, LogOut, ChevronRight } from 'lucide-react'
import { signOut } from '../../../lib/supabase'

const links = [
  { to: '/client',              icon: LayoutDashboard, label: 'نظرة عامة' },
  { to: '/client/automations',  icon: Zap,              label: 'أتمتتي' },
  { to: '/client/leads',        icon: Users2,           label: 'العملاء المحتملون' },
    ...
    <aside className="w-64 flex-shrink-0 flex flex-col h-screen sticky top-0"
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        {links.map(({ to, icon: Icon, label }) => (
          <NavLink key={to} to={to} end={to === '/client'}
            className={({ isActive }) =>
