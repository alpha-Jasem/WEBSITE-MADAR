...
export const ClinicOSSidebar = ({ onClose }: Props) => {
  const { packageType, clinicName, isSubscribed } = useClinicOS()
  ...
  return <aside style={{width:232,background:'#fff',borderLeft:'1px solid #e5e9f2',display:'flex',flexDirection:'column',height:'100vh',direction:'rtl'}}>
    ...
    <div style={{padding:10,borderTop:'1px solid #eef1f6'}}>{isSubscribed&&smartCalls?<div style={{padding:12,borderRadius:8,background:'#eafaf5',border:'1px solid #bdebdc'}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:5}}><strong style={{font:'800 11px Cairo',color:'#08785b'}}>AI Voice + WhatsApp</strong><span style={{font:'800 9px Cairo',color:'#08785b'}}>نشط</span></div><div style={{font:'11px Tajawal',color:'#498574'}}>الاشتراك مفعّل</div></div>:<div style={{padding:13,borderRadius:8,background:'#f7f6ff',border:'1px solid #dedaff'}}><Sparkles size={17} color="#6557d9"/><strong style={{display:'block',margin:'8px 0 4px',font:'900 12px Cairo',color:'#302a72'}}>{isSubscribed?'فعّل الاتصال الذكي':'الحساب المجاني'}</strong><p style={{margin:'0 0 10px',font:'11px/1.55 Tajawal',color:'#706b94'}}>{isSubscribed?'لا تخسر المكالمات خارج الدوام أو وقت انشغال الموظفين.':'عاين النظام ثم اختر الباقة المناسبة لتشغيل الخدمات الذكية.'}</p><NavLink to="/clinic-os/dashboard/plans" onClick={onClose} style={{display:'flex',justifyContent:'center',padding:8,borderRadius:6,background:'#6557d9',color:'#fff',textDecoration:'none',font:'800 11px Cairo'}}>{isSubscribed?'عرض الباقات':'تثبيت الاشتراك'}</NavLink></div>}</div>
  </aside>
}
