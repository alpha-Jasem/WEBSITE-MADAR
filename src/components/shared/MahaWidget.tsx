...
// ── Inner widget UI (must be inside ConversationProvider) ─────────────
function MahaWidgetUI({ pathname }: { pathname: string }) {
  const [isOpen, setIsOpen]           = useState(false)
  const [messages, setMessages]       = useState<Message[]>([])
  const [volume, setVolume]           = useState(80)
  const [feedback, setFeedback]       = useState<FeedbackState>(null)
  const [permissionDenied, setPermissionDenied] = useState(false)
  const [inputBars, setInputBars]     = useState([0, 0, 0, 0, 0, 0, 0])
  const [outputBars, setOutputBars]   = useState([0, 0, 0, 0, 0, 0, 0])
  ...
  const conversation = useConversation({
    onConnect:    () => { setHasGreeted(false); setFeedback(null) },
    onDisconnect: () => { stopVizLoop() },
    onMessage:    ({ message, source }) => {
      setMessages(prev => [...prev, { source, text: message, ts: Date.now() }].slice(-20))
    },
    onError: (err) => console.error('[MahaWidget]', err),
  })
  ...
  // Route change → send contextual update
  useEffect(() => {
    if (prevPathRef.current !== pathname) {
      prevPathRef.current = pathname
      if (conversation.status === 'connected')
        conversation.sendContextualUpdate(`[تحديث سياق] ${getPageContext(pathname)}`)
    }
  ...
  // Contextual update after connect
  useEffect(() => {
    if (conversation.status === 'connected' && !hasGreeted) {
      setHasGreeted(true)
      setTimeout(() => conversation.sendContextualUpdate(getPageContext(pathname)), 800)
    }
  }, [conversation.status, hasGreeted, pathname, conversation])
  ...
  function startVizLoop() {
    function tick() {
      ...
      setInputBars(bars => bars.map((_, i) => Math.min(1, input  * (0.6 + Math.random() * 0.3) * (i % 3 === 1 ? 1.4 : 1))))
      setOutputBars(bars => bars.map((_, i) => Math.min(1, output * (0.6 + Math.random() * 0.3) * (i % 3 === 1 ? 1.4 : 1))))
  ...
  useEffect(() => {
    if (conversation.status === 'connected') startVizLoop()
    else stopVizLoop()
    return () => stopVizLoop()
  }, [conversation.status]) // eslint-disable-line
  ...
  const startConversation = async () => {
    ...
    const firstMessage = getFirstMessage(pathname)
  ...
  return (
    <div dir="rtl" onMouseMove={signalActivity}
      ...
      {isOpen && (
        <div style={{
          position: 'absolute', bottom: 72, left: 0, width: 340,
          ...
          <div style={{ background: 'linear-gradient(135deg,#0f172a,#1e3a5f)', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <button onClick={() => setIsOpen(false)} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 8, padding: '6px 8px', cursor: 'pointer', color: '#94a3b8', fontSize: 16, display: 'flex', alignItems: 'center' }}>✕</button>
          </div>

          {/* Waveform */}
          {isConnected && (
            <div style={{ background: '#f8fafc', padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, borderBottom: '1px solid #e2e8f0' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, flex: 1 }}>
                <div style={{ fontSize: 10, color: '#94a3b8' }}>🎤 أنت</div>
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 36 }}>
                  {inputBars.map((h, i) => <WaveBar key={i} height={h} delay={i * 10} />)}
                </div>
              </div>
          ...
          <div ref={transcriptRef} style={{ flex: 1, maxHeight: 200, overflowY: 'auto', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 8, background: '#f8fafc' }}>
            {messages.length === 0 ? (
              ...
              messages.slice(-8).map(m => (
                <div key={m.ts} style={{ display: 'flex', justifyContent: m.source === 'user' ? 'flex-start' : 'flex-end' }}>
                  <div style={{
                    maxWidth: '80%',
                    background: m.source === 'user' ? '#fff' : '#0f172a',
                    color: m.source === 'user' ? '#1e293b' : '#f1f5f9',
                    padding: '8px 12px',
                    borderRadius: m.source === 'user' ? '12px 12px 12px 4px' : '12px 12px 4px 12px',
          ...
          {/* Quick Actions */}
          {isConnected && (
            <div style={{ padding: '8px 16px', display: 'flex', gap: 6, flexWrap: 'wrap', borderTop: '1px solid #e2e8f0', background: '#fff' }}>
              {[
                ...
                <button key={label} onClick={action} style={{ background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: 20, padding: '4px 12px', fontSize: 12, color: '#475569', cursor: 'pointer', fontFamily: 'inherit' }}>{label}</button>
              ))}
            </div>
          )}

          {/* Controls */}
          <div style={{ padding: '12px 16px', background: '#fff', borderTop: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {isConnected && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <input type="range" min={0} max={100} value={volume} onChange={e => setVolume(Number(e.target.value))}
                  style={{ flex: 1, accentColor: '#0ea5e9', cursor: 'pointer', height: 4 }} />
            ...
            {/* Action Button */}
            {!isConnected ? (
              <button onClick={startConversation} disabled={isConnecting} style={{
                width: '100%',
                background: isConnecting ? '#e2e8f0' : 'linear-gradient(135deg,#0f172a,#1e3a5f)',
                ...
                {isConnecting ? <>⏳ جاري الاتصال...</> : <>🎤 ابدأ المحادثة</>}
              </button>
            ) : (
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => { conversation.sendFeedback(true); setFeedback('positive') }} disabled={feedback !== null} style={{ flex: 1, background: feedback === 'positive' ? '#dcfce7' : '#f1f5f9', border: feedback === 'positive' ? '1px solid #22c55e' : '1px solid #e2e8f0', borderRadius: 10, padding: '8px', cursor: feedback ? 'default' : 'pointer', fontSize: 16 }}>👍</button>
                <button onClick={() => { conversation.sendFeedback(false); setFeedback('negative') }} disabled={feedback !== null} style={{ flex: 1, background: feedback === 'negative' ? '#fee2e2' : '#f1f5f9', border: feedback === 'negative' ? '1px solid #ef4444' : '1px solid #e2e8f0', borderRadius: 10, padding: '8px', cursor: feedback ? 'default' : 'pointer', fontSize: 16 }}>👎</button>
                <button onClick={endConversation} style={{ flex: 2, background: '#fee2e2', border: '1px solid #fecaca', borderRadius: 10, padding: '8px 12px', cursor: 'pointer', color: '#ef4444', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontFamily: 'inherit' }}>📵 إنهاء</button>
              </div>
            )}

            {permissionDenied && (
              <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#c2410c', textAlign: 'center' }}>
      ...
      <button onClick={() => setIsOpen(o => !o)} style={{
        ...
        background: isConnected ? 'linear-gradient(135deg,#0ea5e9,#6366f1)' : 'linear-gradient(135deg,#0f172a,#1e3a5f)',
        border: 'none', cursor: 'pointer',
        boxShadow: isConnected ? `0 4px 20px rgba(14,165,233,0.5), 0 0 0 ${isSpeaking ? '8px' : '3px'} rgba(14,165,233,0.2)` : '0 4px 20px rgba(15,23,42,0.35)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        {isConnecting && (
          <div style={{ position: 'absolute', top: 2, right: 2, width: 12, height: 12, borderRadius: '50%', background: '#f59e0b', border: '2px solid #fff' }} />
        )}
      </button>
...
// ── Outer: provides ConversationProvider + route hooks ────────────────
export function MahaWidget() {
  ...
  const clientTools = {
    ...
    get_current_page: async () => ({
      ...
      context: getPageContext(locationRef.current.pathname),
    }),
  }

  return (
    <ConversationProvider clientTools={clientTools}>
      <MahaWidgetUI pathname={location.pathname} />
