...
export const AiChatWidget = ({ route, accentColor, productName }: Props) => {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      const greet = route.includes('car-wash')
        ? `أهلاً! أنا مساعد مدار لـ Car Wash OS 🚗\nعندي كل المعلومات عن النظام. كيف أقدر أساعدك؟`
        : `أهلاً! أنا مساعد مدار لـ Clinic OS 🦷\nأقدر أشرح لك كيف تشتغل نورة وباقي مميزات النظام. وش تبي تعرف؟`
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 end-6 z-50 w-[340px] sm:w-[380px] rounded-2xl overflow-hidden flex flex-col"
            style={{
      ...
      <motion.button
        ...
        className="fixed bottom-6 end-6 z-50 w-14 h-14 rounded-full flex items-center justify-center cursor-pointer shadow-lg"
