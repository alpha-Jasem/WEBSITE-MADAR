...
import { useClinicAppointments, useClinicMessages, useClinicOpportunities, useClinicTodayAppointments } from '../../../lib/clinicOSQueries'
...
export const DashboardOverview = () => {
  const { userName, packageType, isDemo, isSubscribed, usageMetrics, usageSummary, companyId, accountLoading } = useClinicOS()
  const { data: liveToday = [], loading: appointmentsLoading } = useClinicTodayAppointments(companyId, isDemo)
  ...
  const monthBookings = allAppointments.filter(a => a.appointment_date?.startsWith(month) && !['cancelled','no_show'].includes(a.status)).length
  const summary = isDemo ? AI_RECEPTIONIST_DEMO : {
    ...usageSummary,
    bookings: monthBookings || usageSummary.bookings,
    conversations: new Set(messages.map(m => m.recipient_phone)).size || usageSummary.conversations,
    lostOpportunities: opportunities.filter(o => o.status !== 'closed').length || usageSummary.lostOpportunities,
  }
  ...
  const kpis = [
    ...
    [smart?Phone:Lock,'مكالمات ذكية',smart?String('smartCallMinutes' in summary ? summary.smartCallMinutes : summary.smartCalls):'غير مفعلة',smart?'دقيقة اتصال ذكي مستخدمة':'متاحة في الباقة الأعلى','violet'],
  ...
  const todayAppointments = isDemo ? DEMO_TODAY_APPOINTMENTS : liveToday
  const activities = isDemo ? ['تم حجز موعد بواسطة المساعد الذكي — تنظيف أسنان — 6:40 م','تم تحويل عميل للموظف — استفسار عن التقسيط','تم الرد على عميل خارج الدوام — 11:35 م'] : [
    ...todayAppointments.slice(0, 3).map((a:any) => `موعد ${a.patient_name || a.name || 'مريض'} — ${a.service_name || a.service || 'خدمة'} — ${a.start_time?.slice(0,5) || a.time || ''}`),
    ...messages.slice(0, 2).map(m => `رسالة ${m.status === 'failed' ? 'تعذر إرسالها' : 'تم تسجيلها'} — ${m.recipient_name || m.recipient_phone}`),
    ...opportunities.filter(o => o.status !== 'closed').slice(0, 2).map(o => `فرصة تحتاج متابعة — ${o.customer_name || o.customer_phone}`),
  ].slice(0, 5)
  if (accountLoading) return <div className="clinic-card clinic-empty-state"><Clock3 size={24}/><strong>جاري تجهيز لوحة العيادة</strong><span>نحمّل بيانات الحساب والاشتراك.</span></div>
  return <div className="clinic-ai-page">
    ...
    <div className="clinic-two-col">
      <div className="clinic-card clinic-section"><div className="clinic-section-head"><div><h2>مواعيد اليوم</h2><p>المصدر يوضح كيف وصل كل حجز.</p></div><button className="clinic-action secondary" onClick={()=>navigate('/clinic-os/dashboard/bookings')}>عرض الحجوزات</button></div><div className="clinic-list">{appointmentsLoading?<div className="clinic-empty-state"><Clock3 size={22}/><strong>جاري تحميل المواعيد</strong></div>:todayAppointments.map((a:any)=><div className="clinic-list-row" key={a.id||a.name} style={{gridTemplateColumns:'1fr .45fr .9fr .65fr .8fr'}}><strong>{a.patient_name||a.name}</strong><span style={{font:'800 12px Sora,Cairo'}}>{a.start_time?.slice(0,5)||a.time}</span><span>{a.service_name||a.service}</span><span className="clinic-badge">{a.source}</span><span className={`clinic-badge ${(a.status==='confirmed'||a.status==='مؤكد')?'success':String(a.status).includes('review')?'danger':'warning'}`}>{a.status}</span></div>)}{!appointmentsLoading&&!todayAppointments.length&&<div className="clinic-empty-state"><CalendarCheck size={22}/><strong>لا توجد مواعيد اليوم بعد</strong><span>ستظهر الحجوزات هنا فور تسجيل أول موعد.</span></div>}</div></div>
